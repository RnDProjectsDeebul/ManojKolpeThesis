...
class UNet_en(nn.Module):
    ...
    def forward(self, X, D):
        ...
        D = D.float()
        K = torch.exp(self.gamma2.cpu().float()) * (1 + math.sqrt(3) * D / torch.exp(self.ell.cpu().float())) * torch.exp(-math.sqrt(3) * D / torch.exp(self.ell.cpu().float()))
        I = torch.eye(l).expand(l, l).float()
             
        X, _ = torch.solve(Y, K+torch.exp(self.sigma2.cpu().float())*I)
        Z = K.bmm(X)
        Z = F.relu(Z)
        new_list = []
